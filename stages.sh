...
get_next_stage_number() {
    local tags=$(git tag -l "Tappa-*" 2>/dev/null | sort -V)

    if [ -z "$tags" ]; then
        echo "01"
    else
        local latest=$(echo "$tags" | tail -1)
        local num=$(echo "$latest" | grep -oE '[0-9]{2}' | head -n 1)
        local next=$((10#$num + 1))
        printf "%02d" "$next"
    fi
}
...
tag_command() {
    ...
    # Check if there is already a Tappa-* tag on this commit
    local existing_tags=$(git tag --points-at HEAD | grep "^Tappa-")
    if [ -n "$existing_tags" ]; then
        print_error "This commit already has a Tappa-* tag: $existing_tags"
        exit 1
    fi

    local next_num=$(get_next_stage_number)
    local tag_name="Tappa-$next_num"

    if [ "$next_num" == "01" ]; then
        tag_name="Tappa-01--Finestra-Vuota"
    fi
...
export_command() {
    ...
    if [ -z "$tags" ]; then
        print_error "No Tappa-* tags found"
        exit 1
    fi
    ...
    for tag in $tags; do
        local stage_num=$(echo "$tag" | grep -oE '[0-9]{2}' | head -n 1)
        local stage_dir="FCG_Stages/Tappa-$stage_num"
...
compile_command() {
    ...
    local stages=$(find FCG_Stages -maxdepth 1 -type d -name "Tappa-*" | sort -V)

    if [ -z "$stages" ]; then
        print_error "No Tappa-* directories found in FCG_Stages"
        exit 1
    fi
...
update_command() {
    ...
    local formatted_num=$(printf '%02d' "$stage_num")
    local tag_name="Tappa-$formatted_num"

    if [ "$formatted_num" == "01" ]; then
        tag_name="Tappa-01--Finestra-Vuota"
    fi
